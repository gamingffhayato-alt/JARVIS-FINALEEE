"""
image_generator.py - Diagram Generator (Permanent Vision Solution)
  1. Cloudflare AI (FLUX) generates a clean UNLABELLED illustration.
  2. Cloudflare AI (Vision Model) looks at the GENERATED IMAGE and maps x/y coordinates.
...
async def _generate_base_image(client: httpx.AsyncClient, topic: str) -> Optional[bytes]:
    """Step 1: Generate the unlabelled base image."""
    import re as _re
    clean_topic = _re.sub(
...
async def _get_vision_labels(client: httpx.AsyncClient, image_bytes: bytes, topic: str) -> list[dict]:
    """Step 2: Pass the generated image to a Vision model to get accurate spatial coordinates."""
    b64_image = base64.b64encode(image_bytes).decode('utf-8')

    # Note: Replace with `@cf/meta/llama-3.2-11b-vision-instruct` or your preferred vision model
    ...
    payload = {
        "messages": [
            {
                "role": "user",
                "content": (
                    f"Look at this image of {topic}. Identify up to 8 distinct parts. "
        ...
        "image": [b64_image], # Cloudflare Vision API format for passing images
        "max_tokens": 1024,
        "temperature": 0.1,
    }
...
def _overlay_labels(image_bytes: bytes, labels: list[dict]) -> bytes:
    """Step 3: Draw the labels using the dynamically calculated coordinates."""
    ...
    for item in labels:
        ...
        # Vision model points directly at the anatomy
        ...
        # Calculate connector line target
...
async def generate_diagram(prompt: str) -> Optional[tuple[bytes, str]]:
    ...
    async with httpx.AsyncClient() as client:
        # 1. Image MUST be generated first in the new pipeline
        base_image = await _generate_base_image(client, prompt)
        
        if not base_image:
            return None
        
        # 2. Pass the generated image to the Vision model to map it
